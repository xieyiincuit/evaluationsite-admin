  ...
  <div class="game-type">
    <el-card shadow="always">
      <div class="control-btns">
        <el-button type="primary" @click="handleCreate">录入游戏类型</el-button>
      </div>
    </el-card>
    <el-table
      ref="multipleTable"
...
export default {
  ...
  data() {
    return {
      ...
      formRules: {
        categoryName: [
          { required: true, message: '请输入游戏类型名', trigger: 'blur' },
          {
            min: 0,
            max: 50,
            message: '不应超过50个字符',
  ...
  methods: {
    ...
    fetchData() {
      ...
      // 获取数据列表接口
      this.$http.get(
        `/v1/g/categories?pageIndex=${this.listQuery.currentPage}`,
        null,
        (res) => {
        ...
        if (valid) {
          ...
          if (this.isEdit) {
            const categoryUpdateDto = {
              ...
              categoryName: this.dialogForm.categoryName
            }
            this.$http.put(
              'v1/g/category',
              categoryUpdateDto,
